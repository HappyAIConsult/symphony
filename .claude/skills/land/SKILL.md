...
> Automated AI code-review (e.g. `## Codex Review` / `## Claude Review` bot
> comments) is **off by default**. Human reviews and CI checks are always
> honored. Enable an AI reviewer only when a CI workflow actually posts review
> comments, via env vars read by `land_watch.py`:
> `SYMPHONY_LAND_WAIT_FOR_AI_REVIEW=true`,
> `SYMPHONY_LAND_AI_REVIEW_MARKER="## Claude Review"`,
> `SYMPHONY_LAND_AI_REVIEW_BOTS="<bot-login>"`. The agent prefixes its own
> replies with `[claude]` (override: `SYMPHONY_LAND_AGENT_PREFIX`).
